...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        expression2bind = expressions.rest
        if isinstance(expression2bind, Pair) and expression2bind.__len__() > 1:
            expression2bind = scheme_eval(expression2bind, env)
        elif isinstance(expression2bind, Pair) and expression2bind.__len__() == 1:  #pair是单个数字，比如(2)
            expression2bind = scheme_eval(expression2bind.first, env)
        
        env.define(symbol, expression2bind)
        return symbol
        "*** YOUR CODE HERE ***"
        # END PROBLEM 4
        ...
        env.define(symbol, do_lambda_form(Pair(signature.rest, expressions.rest), env))
        return symbol
        # END PROBLEM 10
    else:
